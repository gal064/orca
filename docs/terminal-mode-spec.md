...
_starting_ directory (immaterial after launch) and whose workspace key
...
- `src/renderer/src/store/slices/terminal-cwd.ts` — ptyId → cwd map fed by a
  ...
  `activePwdByVtab` = last-known cwd of the most-recently-focused _terminal_
...
1. `components/sidebar/index.tsx` — one conditional: terminal mode renders
   ...
   in _both_ modes (its data outlives the flag), and it cannot be split out of
   ...
   - `components/WorktreeJumpPalette.tsx` (Cmd+J group rows _and_ their ordering base).
...
3. **Git panel follows the pwd's repo (Phase 3, done).**
   ...
   path through ~150 call sites, terminal mode substitutes the _workspace pair_
   ...
   (`src/shared/protocol-version.ts` — defined and checked, deliberately _not_
   ...
   the host by its _workspace_ root while the panels show the pwd.
...
8. Settings registration (Phase 0, done): `src/shared/constants.ts`
   (`getDefaultSettings` — _not_ `persistence.ts`, which only migrates),
...
11. **PTY disposal on folder-workspace delete (Phase 1, done — resolved
    ...
    `resolveFolderWorkspaceConnectionId` — the same resolver the pty _spawn_ path
...
- Terminal-mode-owned: `store/terminal-mode-ssh-connect.ts` connects the SSH target
  ...
  _reconnect_ UI budget; `store/slices/vertical-tabs.ts` rejects an unresolved home
...
- `src/main/ipc/pty.ts` — a 30-line `resolveRestoredTabFallbackCwd` plus its call site:
  ...
  when the requested cwd _is_ that tab's `lastCwd`, so no classic spawn (split-pane
...
- `components/terminal-pane/use-terminal-pane-lifecycle.ts` — one substitution in the
  ...
  (`lib/terminal-tab-start-cwd.ts`) reads the tab from the store _snapshot_. Not the
...
16. **Agents entry gate (macOS acceptance fix, done).** `vertical-tabs/VerticalTabsAgentsEntry.tsx`
    ...
    `SidebarNav.test.tsx` renders _both_ rows in both flag states, so the shared gate is a
    conclusion rather than an assumption.

17. **Phase 8 — macOS QA polish (done).** Four exploratory-QA findings. New files:
    `main/ipc/terminal-mode-startup-cwd-home.ts`,
    `renderer/src/components/terminal-pane/startup-cwd-fallback-notice.ts`,
    `renderer/src/store/terminal-mode-empty-workspace.ts`,
    `renderer/src/store/terminal-mode-workspace-keys.ts` and
    `renderer/src/store/workspace-catalog-cache.ts` (the pure half of the catalog choke
    point, split out so store slices can read it without pulling the store into their own
    module init),
    `renderer/src/components/vertical-tabs/TerminalModeEmptyPane.tsx`,
    `renderer/src/components/settings/notification-test-feedback.ts`. Upstream edits:

- `src/shared/terminal-startup-cwd.ts` — the missing-directory fallback became a
  _usability_ chain: the injected probe answers `'usable' | 'missing' | 'inaccessible'`
  (a `chmod 000` directory stats fine, and the child dies on `chdir`), the chain gained
  a lazily-resolved `homeCwd` last resort, and it now returns a result object rather
  than a bare path so the caller learns _which_ tier answered. Only a vertical tab is
  offered `homeCwd`, so a classic workspace on an unmounted volume still surfaces the
  provider's own error. Remote and SSH tabs are out of the chain's reach — see the
  design doc's Phase 8 note.
- `src/main/ipc/pty.ts` — `localStartupCwdUsability` (adds an `accessSync(X_OK)` probe
  to the existing `statSync`), the `homeCwd` argument, and two `console.warn`s. Main had
  no trace of a doomed spawn at all: node-pty succeeds and the _child_ fails its chdir.
- `src/preload/api-types.ts`, `src/preload/index.ts`,
  `components/terminal-pane/pty-transport-types.ts` — `startupCwdFallback` is now the
  shared `TerminalStartupCwdFallbackNotice` (`kind: 'worktree' | 'home'`, optional
  `reason`). Preload↔main only; no RPC method, stream opcode or relay shape changes.
- `components/terminal-pane/pty-connection.ts` — two branch points: the notice text moved
  out to `startup-cwd-fallback-notice.ts`, and the newborn-keep-alive guard gained
  `!isTerminalModeVerticalTabKey(...)` so a vertical tab's dead shell always closes its tab.
- `store/slices/tabs.ts`, `components/terminal/terminal-tab-actions.ts`,
  `components/terminal-pane/TerminalPaneOverlayLayer.tsx`,
  `components/tab-group/useTabGroupWorkspaceModel.ts` and `components/Terminal.tsx`
  (browser-tab close) — one condition each, `isTerminalModeVerticalTabKey`, so an
  emptied vertical tab keeps its selection. The overlay layer is the one a dying shell
  actually reaches; the other four are the user-close, split-group and browser-close
  paths, guarded so no close path disagrees.
- `App.tsx` — one selector (`selectTerminalModeMainPaneState`, read through
  `useShallow`), one term in `terminalWorkbenchVisible`, and the `Landing` slot branching
  to `<TerminalModeEmptyPane/>`. `Landing` now renders only for `kind: 'classic'`, so the
  cold-start window cannot fall through to it.
- `store/slices/terminals.ts` — `openNewTerminalTabInActiveWorkspace` takes an optional
  `groupId` and resolves its own default, so the empty pane reuses that funnel (and its
  web-runtime routing) instead of forking a `createTab` for a workspace whose groups
  collapsed with its last tab.
- `lib/terminal-mode.ts` — `isTerminalMode` resolves the platform only after the flag
  check; it is now on every store write and the platform read crosses the context bridge.
- `components/settings/NotificationsPane.tsx` — an inline result beside "Send Test
  Notification" (not terminal-mode, carried on this branch).
- `components/terminal-pane/replay-guard.ts` — a disposed terminal releases quietly
  instead of certifying the write pipeline dead (also not terminal-mode).
...
**existing** stream/RPC shape or a new _optional_ field — no new opcode without
...
3. **Remote reattach proof:** kill the client mid-session, reconnect; all
   ...
   _host_ while the client is up — was the hard blocker; **Phase 5 task 0 lands it
   ...
   One defect remains on that path and is _not_ a blocker for the switch-over: the
